...
public:
	...
	public:
		...
		MyLoop(SizeType total,
		       std::vector<RealType> &vobs1,
		...
		void doTask(SizeType taskNumber, SizeType)
		{
			params.m = taskNumber;
			RealType* ptr = &(pts[0]);
			gslWrapper_.gsl_integration_qagp(&f,
			                                 ptr,
			                                 pts.size(),
			                                 epsabs,
			                                 epsrel,
	...
	public:
		...
		SizeType tasks() const { return matrix.rows(); }
	...
	public:
		...
		SizeType tasks() const { return info_.top(); }
	                ...
	                const BaseFunctorType& obsFunc) const
	{
		...
		const SizeType threads = std::min(tpemParameters_.cutoff,
		...
		HelperType helper(tpemParameters_.cutoff, vobs, obsFunc, gslWrapper_);
	                 ...
	                 std::vector<RealType>& moment) const
	{
		...
		const SizeType threads = std::min(matrix.rows(),
		                                  PsimagLite::Concurrency::codeSectionParams.npthreads);
		PsimagLite::CodeSectionParams codeSectionParams(threads);
		Parallelizer2Type threadObject(codeSectionParams);
	...
	{
		assert(matrix0.rows()==matrix0.cols());
		TpemSubspaceType info(matrix0.rows());
		...
		assert(matrix0.rows()==matrix0.cols());
		moment0[0] = moment1[0] =  matrix0.rows();

		typedef MyLoop3 Helper3Type;
		typedef PsimagLite::Parallelizer<Helper3Type> Parallelizer3Type;
		const SizeType threads =// std::min(info.top(),
		        PsimagLite::Concurrency::codeSectionParams.npthreads;
		PsimagLite::CodeSectionParams codeSectionParams(threads);
		Parallelizer3Type threadObject(codeSectionParams);
...
private:
	...
	static void diagonalElement(const TpemSparseType& matrix,
	                            ...
	                            const TpemParametersType& tpemParameters)
	{
		TpemSubspaceType work(matrix.rows());
	...
	{
		assert(matrix.rows()==matrix.cols());
		std::vector<RealOrComplexType> tmp(matrix.rows(),0.0);
		std::vector<RealOrComplexType> jm0(matrix.rows(),0.0);
		std::vector<RealOrComplexType> jm1(matrix.rows(),0.0);
		...
		RealOrComplexType sum1 = PsimagLite::conj(jm0[ket]) *  jm1[ket];
		moment[1] += std::real(sum1);

		RealOrComplexType sum2 = 0.0;
		for (SizeType p = 0; p < info.top(); p++)
			sum2 += PsimagLite::conj(jm1[info(p)]) * jm1[info(p)];
		...
		for (SizeType m = 2; m < n / 2; m++) {
			...
			for (SizeType p = 0; p < info.top(); p++) {
				...
				/* for moment[2 * m    ] */
				sum1 += PsimagLite::conj(keep) * keep;
				/* for moment[2 * m - 1] */
				sum2 += PsimagLite::conj(keep) * jm1[i];
	                                ...
	                                SizeType ket)
	{
		...
		RealOrComplexType sum2 =  0.0;
		for (SizeType j=0;j<matrix.rows();j++)
			sum2 += PsimagLite::conj(jm1[j]) * jm1[j];
		moment[2] += std::real (sum2);
	                             ...
	                             const TpemParametersType& tpemParameters)
	{
		...
		assert(matrix0.rows()==matrix0.cols());
		TpemSubspaceType work(matrix0.rows());
