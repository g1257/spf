...
namespace Spf {
	...
	public:	
		...
		size_t getSubInterval(const RealType& x) const
		{
			size_t n = size_t(steps_*(x-minE_));
			return size_t(n/(maxE_-minE_));
		}

		void add(const RealType& x,const FieldType& y)
		{
			size_t n = getSubInterval(x);
		...
		void multiply(const RealType& x,const FieldType& y)
		{
			size_t n = getSubInterval(x);
			// Don't remove this checking it's very important!
			if (n>=steps_ || x<minE_) {
